...
| **Linux · X11** | 🟡 best-effort, **measured** | Everything the overlay needs works with no platform-specific code: `_NET_WM_STATE_ABOVE`, `set_position`, `set_ignore_cursor_events`, and a 32-bit ARGB visual. **A compositing manager is a hard requirement**, not a nicety — without one the window renders 93 % flat black and the pet is a solid rectangle over the user's work, which fails §1.1 as surely as being invisible does. The app detects it via `gdk_screen_is_composited` and warns at startup. Verified on Xvfb + openbox; not on GNOME or KDE. See [Spike E findings](artifacts/spike-e/FINDINGS.md). |
| **Linux · Wayland** | ❌ **unsupported, decided** | Confirmed by measurement, not just by reading the protocol: the app runs and the webview connects, but `primary_monitor()` returns nothing so the pet cannot compute a corner and lands at `0,0`, and no protocol lets a client raise itself. `wlr-layer-shell` could, but Tauri does not use it and GNOME does not implement it. A pet fixed top-left behind the terminal fails §1.1, so Linux ships X11-only and the app **says so on stderr at startup** rather than failing silently. |
...
- [x] **Spike E (Linux):** run on Xvfb + openbox and on weston, not on GNOME or KDE — two standard-conforming compositors rather than the four named desktops. Decided: **X11 supports every capability; Wayland supports none of the ones that matter, and Linux ships X11-only with a startup warning** (§3.1). It also found a bug that was never about Linux: `clamp_to_visible` was measuring a zero window size, so the check that exists to keep the pet reachable would have approved a position 90 % off-screen. See [Spike E findings](artifacts/spike-e/FINDINGS.md).
