...
| **Linux · X11** | 🟡 best-effort, **measured** | Everything the overlay needs works, with no platform-specific code: `_NET_WM_STATE_ABOVE`, `set_position`, `set_ignore_cursor_events`, and a 32-bit ARGB visual for transparency (which still needs a compositing manager running — openbox alone draws it opaque). Verified on Xvfb + openbox; not on GNOME or KDE. See [Spike E findings](artifacts/spike-e/FINDINGS.md). |
